...
public class GenericDAO extends HibernateDaoSupport
{
   ...
   * JSR-220 semantics. Similar to saveOrUpdate, but never associates the given object with the
   * current Hibernate Session. In case of a new entity, the state will be copied over as well.
   * 
   * Note that merge will not update the identifiers in the passed-in object graph (in contrast to
   * TopLink)! Consider registering Spring's IdTransferringMergeEventListener if you would like to
   ...
   * exception if not found. This method is a thin wrapper around HibernateTemplate.load(Class,
   * java.io.Serializable) for convenience. For an explanation of the exact semantics of this
   ...
   * not found. This method is a thin wrapper around HibernateTemplate.get(Class,
   * java.io.Serializable) for convenience. For an explanation of the exact semantics of this
   * method, please do refer to the Hibernate API documentation in the first instance.
   * 
   * @param <T>
   ...
   * or null if not found. This method is a thin wrapper around
   * HibernateTemplate.findByCriteria(DetachedCriteria criteria, int firstResult, int
   * maxResults)convenience. For an explanation of the exact semantics of this method, please do
  ...
  /**
   * Delete all given persistent instances. This can be combined with any of the find methods to
   * delete by query in two lines of code.
   * 
   * @param <T>
   *          a persistent generics
   * @param entities
   ...
   */
  @SuppressWarnings("unchecked")
  public <T> List<T> findPagedDateByDetachedCriteria(final DetachedCriteria detachedCriteria, final int pageSize, final int startPos)
  {
